...
class CategoryDetailEdit extends React.Component {
  ...
  componentWillMount() {
    if (this.state.id !== undefined) {
      this.props.sendData(this.handleInitData);
    }
  ...
  render() {
    ...
    // 处理数据，将后台数据转为字符串，匹配默认值
    let pidStr;
    if (initialData.PID !== undefined) {
      pidStr = initialData.PID.toString();
    }
    let swNameIdStr;
    if (initialData.SW_ID !== undefined) {
      swNameIdStr = initialData.SW_ID.toString();
    }
    let moduleIdStr;
    if (initialData.SW_MODULE_ID !== undefined) {
      moduleIdStr = initialData.SW_MODULE_ID.toString();
      console.log('SW_MODULE_ID', moduleIdStr);
    }
    let developUnitId;
    if (initialData.SW_DEPARTMENT_ID !== undefined) {
      developUnitId = initialData.SW_DEPARTMENT_ID.toString();
    }
    let ownerId;
    if (initialData.SU_OWNERID !== undefined) {
      ownerId = initialData.SU_OWNERID.toString();
    }
    // console.log(pidStr, swNameIdStr, moduleIdStr);
    let toWhere;
    if (this.props.location.pathname === '/main/SoftwareInfo/CategoryDetailAdd') {
...
CategoryDetailEdit.propTypes = {
  ...
  // generateTree: PropTypes.func.isRequired,
