  ...
  render() {
    ...
    const dataHead = [
      ...
      },
      {
        ...
        defaultValue.dueTime.map(
          ele => moment(ele).format('YYYY年MM月DD日'),
        ).join('-'),
      },
      {
        key: '创建人',
    ...
    return (
      <Row>
        <Col span="24">
          ...
          >返回</Button>
          <Button onClick={() => browserHistory.push(`/main/SoftwareDeployment/Scheme/Edit/${activeID}`)}>编辑</Button>
...
SchemeView.propTypes = {
  ...
  activeID: React.PropTypes.invalid,
  // jsonsTemp: React.PropTypes.array,
};
