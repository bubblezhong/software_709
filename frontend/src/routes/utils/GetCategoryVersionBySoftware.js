...
class GetCategoryVersionBySoftware extends React.Component {
  ...
  componentWillMount() {
    ...
    this.props.getData(this.props.id, params, (res) => { this.handleData(res); });
  }
      ...
      <div>
        {treeNodes.length !== 0 ?
          <Tree
            ...
            defaultExpandAll
          >
            <TreeNode title={`${this.props.softwareDetail.NAME}${this.props.softwareDetail.SW_VERSION}`} key="id0">
              {treeNodes}
            </TreeNode>
          </Tree> :
            <span />
